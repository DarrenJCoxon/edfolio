 ...
 *
 * Rephrase selected text using Scaleway AI.
 *
 * Request body:
 ...
 *
 * Response:
 * - 200: { data: { originalText: string, rephrasedText: string } }
 * - 401: Unauthorized
 * - 400: Invalid input
 * - 403: Access denied
...
export const POST = withCsrfProtection(async (request: NextRequest) => {
  try {
    ...
    const result = await scalewayClient.chat({
      ...
      temperature: 0.7,
      // Estimate tokens: ~4 chars per token, so text.length/4 input tokens
      // Allow 2x the input length for output to avoid truncation
