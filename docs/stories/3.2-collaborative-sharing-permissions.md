...
- Access: Only invited email addresses can view (with valid token + authentication)
- File Browser: Appears in recipient's "Shared with Me" virtual folio
...
- Access: Only invited email addresses can edit (with valid token + authentication)
- File Browser: Appears in recipient's "Shared with Me" virtual folio
...
### "Shared with Me" Virtual Folio
- [ ] Create virtual system folio called "Shared with Me"
- [ ] Appears in folio switcher above user's personal folios
- [ ] Shows all pages where user has been granted token-based access (read or edit)
- [ ] Virtual folio has special ID: `__shared__` and flag `isSystem: true`
- [ ] Displays: page title, sharer name, permission level, last accessed date
- [ ] Badge showing count of shared items
- [ ] Different icon (Users icon) to distinguish from regular folios

### First-Time Token Access Flow
...
### Clone Capability
- [ ] Clone button/option available for shared pages (read or edit)
- [ ] Clone requires user to be authenticated
- [ ] Clone creates independent copy in user's default folio
- [ ] Cloned page is private by default (not published)
...
**NEW FEATURE:** Shared pages must appear in recipient's file browser in a special "Shared with Me" folio (similar to Google Drive).
...
**Subtask 17.2: Update Folios API to Include Virtual Folio (30 mins)**
- [ ] Open `/app/api/folios/route.ts`
- [ ] In GET endpoint, after fetching user's real folios:
  - Fetch count of shared pages via PageCollaborator query
  - If count > 0, prepend virtual folio to response:
    ```typescript
    {
      id: '__shared__',
      name: 'Shared with Me',
      isSystem: true,
      icon: 'Users', // Special icon identifier
    ...
    ```
- [ ] Ensure virtual folio always appears first in list
- [ ] Document in code comments that `__shared__` is a special virtual ID

**Subtask 17.3: Auto-Create PageCollaborator on First Token Access (30 mins)**
- [ ] Open `/app/api/public/[slug]/access/route.ts` (from Task 16)
- [ ] After validating token and session, check if PageCollaborator exists:
  ```typescript
...
#### Issue 1: Hardcoded Colors in PermissionBadge - FIXED ✅
...
**Final Breakdown:**
- Acceptance Criteria: 44/44 (100%) ✅
- Code Quality: 10/10 ✅ (Previously 9/10)
- Security: 10/10 ✅
- Performance: 10/10 ✅
- GDPR Compliance: 10/10 ✅
- Documentation: 10/10 ✅
- Standards Compliance: 10/10 ✅ (Previously FAIL)
...
- ✅ All data stored in UK/EU (Prisma Postgres EU, Railway Europe)
- ✅ Email service uses EU infrastructure (Resend EU region)
- ✅ Access tokens cryptographically secure (256-bit)
- ✅ Configurable token expiry implemented
- ✅ User can request deletion of invitation history (cascade delete)
- ✅ No unnecessary PII collected (only email addresses)
- ✅ Unsubscribe link present in all emails
- ✅ Data processing transparent (invitation emails explain purpose)
- ✅ User consent implicit through invitation acceptance
- ✅ Right to access: Users can view their shares via "Shared with Me"
- ✅ Right to erasure: Owner can revoke access, deleting share records
